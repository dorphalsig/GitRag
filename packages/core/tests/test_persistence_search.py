...
def test_libsql_keyword_relevance_handles_fractional_and_negative_scores() -> None:
    assert PersistInLibsql._keyword_relevance(-0.125) == 0.125
    assert PersistInLibsql._keyword_relevance(0.0) == 1.0
    assert PersistInLibsql._keyword_relevance(1.0) == 0.5


def test_libsql_search_uses_fractional_keyword_score_without_truncation() -> None:
    engine = create_engine("sqlite:///:memory:", future=True)
    _bootstrap_libsql_schema(engine)
    cfg = LibsqlConfig.from_parts(database_url="file:memdb5")
    adapter = PersistInLibsql(cfg=cfg, dim=2, engine=engine)

    # both vectors are equivalent to keep ranking dependent on keyword score
    c1 = _chunk(repo="r", path="low.py", body="keyword", vec=[1.0, 0.0])
    c2 = _chunk(repo="r", path="high.py", body="keyword", vec=[1.0, 0.0])
    adapter.persist_batch([c1, c2])

    fake_rows = [
        {
            "repo": "r", "path": "low.py", "chunk": "keyword", "language": "python",
            "start_row": 0, "start_col": 0, "end_row": 0, "end_col": 1,
            "start_bytes": 0, "end_bytes": 7, "embedding": c1.embeddings, "keyword_score": -0.01,
        },
        {
            "repo": "r", "path": "high.py", "chunk": "keyword", "language": "python",
            "start_row": 0, "start_col": 0, "end_row": 0, "end_col": 1,
            "start_bytes": 0, "end_bytes": 7, "embedding": c2.embeddings, "keyword_score": -0.30,
        },
    ]

    with mock.patch.object(adapter, "_keyword_search", return_value=fake_rows):
        out = adapter.search(query_embedding=[1.0, 0.0], query_text="keyword", limit=2)

    assert [c.path for c in out] == ["high.py", "low.py"]

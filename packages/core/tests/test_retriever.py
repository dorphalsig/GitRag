...
def test_qwen3_reranker_reload_when_model_or_attention_changes() -> None:
    with mock.patch("transformers.AutoTokenizer.from_pretrained", return_value=mock.MagicMock()) as tok_loader, mock.patch(
        ...
        Qwen3Reranker(model_name="model-a", attn_implementation="eager")
        Qwen3Reranker(model_name="model-a", attn_implementation="sdpa")
        Qwen3Reranker(model_name="model-b", attn_implementation="sdpa")

    # 1st load, plus reload for attn change, plus reload for model change
    assert tok_loader.call_count == 3
    assert model_loader.call_count == 3
